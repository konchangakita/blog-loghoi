...
### 8.4 権限管理
- **Dockerコンテナ権限**: `user: "1000:1000"` でnutanixユーザーのUID:GIDを設定
- **ファイル作成権限**: コンテナ内で作成されるファイルが適切な権限（nutanix:nutanix）で作成される
- **アクセス権限**: ホスト側のnutanixユーザーからすべてのファイルにアクセス可能
- **権限問題回避**: Docker Compose起動時の権限エラーを防止
