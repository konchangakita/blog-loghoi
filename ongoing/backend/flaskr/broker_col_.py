...
class HoiGateway():
    ...
    for root, _, filenames in os.walk(output_directory):
      for filename in filenames:
        ...
        if filename.endswith('.gz'):
            ...
            _hoihoilist.append({'zip_filename': filename, 'directory': os.path.dirname(relative_path), 'file_list': file_list})
  ...
  def hoihoi(self, cvm):
    ...
    log_folder = datetime.datetime.now().strftime(OUTPUT_PATH+"/loghoi_%Y%m%d_%H%M%S")
    ...
    for item in download_list:
      ...
      command = ["scp", "-q", "-o", "StrictHostKeyChecking=no", "-i", key_file, f"{remote_user}@{remote_host}:{remote_path}", local_path]
      try:
